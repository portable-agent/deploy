...
Обычный запуск сохраняет детерминированную demo-модель. Для ручной проверки естественной речи через
установленную на хосте Ollama используй:

```powershell
task services:model:up
```

После запуска профиль можно проверить отдельным black-box сценарием с естественной русской фразой:

```powershell
task test:model
```

Команда проверяет путь до локальной модели, разбор относительной даты и создание действия, но не
входит в обычный CI: на CPU ответ может занимать десятки секунд.

Профиль направляет Agent Runtime на `host.docker.internal:11434`, выбирает `qwen2.5:7b` и увеличивает
тайм-ауты только на пути до модели. Значения можно переопределить в локальном `.env`. Ollama и модель
не входят в Compose: разработчик управляет ими отдельно, а CI остаётся быстрым и воспроизводимым.
...
Для настоящего Telegram вместе с распознаванием обычной речи:

```powershell
task telegram:real:model:up
```
